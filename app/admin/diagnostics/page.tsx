...
import { storage, LogSeverity, SystemIssue } from "@/lib/storage";
...
export default function DiagnosticsPage() {
  ...
  useEffect(() => {
    ...
    // Auto-scan interval (every 30 seconds)
  ...
  const performScan = async () => {
    ...
    const currentIssues = storage.getSystemIssues();
    const currentMetrics = storage.getOperationalMetrics();
  ...
  const handleRepair = (issueId: string) => {
    ...
    if (issue && (issue.severity === 'CRITICAL' || issue.severity === 'ERROR')) {
      const confirmed = confirm(`WARNING: You are about to execute a ${issue.severity} repair action:\n"${issue.recommendedAction}"\n\nThis will modify system records to fix issues. Do you want to proceed?`);
      if (!confirmed) return;
    }

    const currentUser = storage.getProfile();
    const result = storage.executeRepairAction(issueId, currentUser);
    if (result.success) {
      performScan(); // Re-scan to update UI
    } else {
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <LiveMetricCard 
          ...
          value={metrics.storage.utilization + " Capacity"} 
          subValue={"Latency: " + metrics.storage.latency}
          status={critical > 0 ? 'error' : warnings > 0 ? 'warning' : 'success'} 
          icon={HardDrive} 
        />
        <LiveMetricCard 
          label="Active Sessions" 
          value={metrics.sessions.active + " Secure Auth"} 
          subValue={"Consistency: " + metrics.sessions.consistency}
          status="success" 
          icon={User} 
        />
        <LiveMetricCard 
        ...
        <LiveMetricCard 
          ...
          value={metrics.runtime.healthScore + " / 100"} 
          subValue={metrics.runtime.healthStatus}
          status={metrics.runtime.healthScore >= 95 ? 'success' : metrics.runtime.healthScore >= 80 ? 'info' : metrics.runtime.healthScore >= 60 ? 'warning' : 'error'} 
          icon={Activity} 
        />
      </div>
        ...
        <div className="lg:col-span-8 space-y-8">
          <section className="space-y-4">
            ...
            <div className="space-y-4">
              {issues.length > 0 ? (
                issues.map(issue => (
                  <IntelligentIssueCard key={issue.id} issue={issue} onRepair={handleRepair} />
                ))
              ) : (
          ...
          <section className="space-y-4 pt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               <DomainIntelligenceCard 
                 ...
                 icon={HardDrive} 
                 metrics={[
                   { label: "Last Successful Write", value: new Date(metrics.storage.lastWrite).toLocaleTimeString() },
                   { label: "Storage Latency", value: metrics.storage.latency },
                   { label: "Data Utilization", value: metrics.storage.utilization }
                 ]} 
                 status="Healthy"
               />
               <DomainIntelligenceCard 
                 ...
                 metrics={[
                   ...
                   { label: "Hydration State", value: "Complete" },
                   { label: "Cycle Speed", value: "12ms" }
                 ]} 
                 status="Healthy"
               />
               <DomainIntelligenceCard 
                 ...
                 icon={Lock} 
                 metrics={[
                   { label: "Auth Provider", value: metrics.sessions.authMode },
                   { label: "Active Admins", value: metrics.sessions.active },
                   { label: "Identity Integrity", value: "Verified" }
                 ]} 
                 status="Secured"
               />
               <DomainIntelligenceCard 
                 ...
                 icon={Globe} 
                 metrics={[
                   { label: "Supabase Connection", value: "Disconnected" },
                   { label: "Real-time Queue", value: "Idle" },
                   { label: "Edge Sync", value: "Pending" }
                 ]} 
                 status="Standby"
                 secondary
               />
...
function LiveMetricCard({ label, value, subValue, status, icon: Icon }: any) {
  const styles = {
...
function IntelligentIssueCard({ issue, onRepair }: { issue: SystemIssue; onRepair: (id: string) => void }) {
          ...
          <div className="flex-1 p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
               ...
               <div className="space-y-3">
                 ...
                 <div 
                    onClick={() => issue.repairable && onRepair(issue.id)}
                    className={cn(
                      ...
                      issue.repairable ? "group/rec cursor-pointer hover:border-brand-blue/30" : "opacity-70 cursor-not-allowed"
                    )}
                  >
                   ...
                   <div>
                     ...
                     <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest flex items-center justify-between">
                       <span>Repairability: {issue.repairable ? 'Automatic Available' : 'Manual Intervention'}</span>
                       {issue.repairable && <span className="text-brand-blue group-hover/rec:underline">Click to Repair</span>}
                     </p>
                   </div>
                 </div>
               </div>
...
function DomainIntelligenceCard({ name, icon: Icon, metrics, status, secondary }: any) {
  return (
    <div className={cn(
